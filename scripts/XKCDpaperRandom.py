...
"""
This script goes to a random xkcd comic, download the image, pastes it on an image with a sweet gradient, then adds it to a folder twice
...
def doSave(link, gradientColors, gradientNodes, doGradient):
    ...
    #this randomly selects a comic from those that haven't been set before
def getComicNumber():
    #first create the file if it doesn't exist
    try:
        f = open("C:\Users\Public\unreadComics.txt", "r")
        f.close()
    except:
        f = open("C:\Users\Public\unreadComics.txt", "w")
        f.close()
    
    #Now get a list of all the unread comics marked in the file. If it is empty, mark all the comics unread.
    unreadComics = open("C:\Users\Public\unreadComics.txt", "r")
    nums = unreadComics.readlines()
    if nums == []:
        for n in range(1, calendarLib.howManyXKCDs() + 1):
            nums.append(str(n) + "\n")
        print "log file empty. all comics seen. starting anew."
    else:
        print "log file not empty. picking from list of unseen comics."
    unreadComics.close()
    comicNum = nums[int(random.random()*len(nums))] #get a random line from the file contents
    nums.remove(comicNum)
    unreadComics = open("C:\Users\Public\unreadComics.txt", "w")
    for num in nums:
        unreadComics.write(num)
    unreadComics.close()
    return comicNum.split("\n")[0]


"""-----------------------------------------START OF PROGRAM---------------------------------------------------------------"""


url = "https://xkcd.com/" + getComicNumber() + "/" #randomly picks a comic from list of unread comics located in C:\Users\Public
print "Comic: " + url
